...
EurocDatasetReader::EurocDatasetReader(const std::string &euroc_path) {
    ...
    for (auto &item : camera_csv.items) {
        image_data.emplace_back(item.t, euroc_path + "/cam0/data/" + item.filename);
...
std::shared_ptr<pvio::Image> EurocDatasetReader::read_image() {
    if (image_data.empty()) {
        return nullptr;
    }
...
std::pair<double, pvio::vector<3>> EurocDatasetReader::read_gyroscope() {
...
std::pair<double, pvio::vector<3>> EurocDatasetReader::read_accelerometer() {
