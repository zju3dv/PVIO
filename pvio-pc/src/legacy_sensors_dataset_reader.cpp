...
class LegacySensorsDataParser : public libsensors::Sensors {
  public:
    LegacySensorsDataParser(LegacySensorsDatasetReader *reader) :
        reader(reader) {
    }
  ...
  protected:
    ...
    void on_accelerometer(double t, double x, double y, double z) override {
        reader->pending_accelerometers.emplace_back(t, pvio::vector<3>{GRAVITY_NOMINAL * x, GRAVITY_NOMINAL * y, GRAVITY_NOMINAL * z});
    }
  ...
  private:
    LegacySensorsDatasetReader *reader;
};

LegacySensorsDatasetReader::LegacySensorsDatasetReader(const std::string &filename) :
    datafile(filename.c_str(), std::ifstream::in | std::ifstream::binary) {
    if (!datafile) {
        std::cout << "Cannot open file " << filename << std::endl;
        exit(EXIT_FAILURE);
    }
    sensors = std::make_unique<LegacySensorsDataParser>(this);
}

LegacySensorsDatasetReader::~LegacySensorsDatasetReader() = default;

DatasetReader::NextDataType LegacySensorsDatasetReader::next() {
...
std::shared_ptr<pvio::Image> LegacySensorsDatasetReader::read_image() {
    std::shared_ptr<pvio::Image> image = pending_images.front();
    pending_images.pop_front();
    return image;
}

std::pair<double, pvio::vector<3>> LegacySensorsDatasetReader::read_gyroscope() {
    std::pair<double, pvio::vector<3>> gyroscope = pending_gyroscopes.front();
    pending_gyroscopes.pop_front();
    return gyroscope;
}
std::pair<double, pvio::vector<3>> LegacySensorsDatasetReader::read_accelerometer() {
