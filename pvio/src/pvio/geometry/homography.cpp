...
bool decompose_homography(const matrix<3> &H, matrix<3> &R1, matrix<3> &R2, vector<3> &T1, vector<3> &T2, vector<3> &n1, vector<3> &n2) {
    ...
    if (is_pure_rotation) {
        // Pure rotation
        Eigen::JacobiSVD<matrix<3>> svd(H, Eigen::ComputeFullU | Eigen::ComputeFullV);
...
namespace {
    ...
    inline matrix<3> solve_homography_normalized(const std::array<vector<2>, 4> &pa, const std::array<vector<2>, 4> &pb) {
