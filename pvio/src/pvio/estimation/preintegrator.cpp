...
void PreIntegrator::increment(double dt, const ImuData &data, const vector<3> &bg, const vector<3> &ba, bool compute_jacobian, bool compute_covariance) {
...
bool PreIntegrator::integrate(double t, const vector<3> &bg, const vector<3> &ba, bool compute_jacobian, bool compute_covariance) {
...
void PreIntegrator::compute_sqrt_inv_cov() {
    delta.sqrt_inv_cov = Eigen::LLT<matrix<15, 15>>(delta.cov.inverse()).matrixL().transpose();
}
...
void PreIntegrator::predict(const Frame *old_frame, Frame *new_frame) {
    ...
    new_frame->pose.p = old_frame->pose.p + 0.5 * gravity * delta.t * delta.t + old_frame->motion.v * delta.t + old_frame->pose.q * delta.p;
