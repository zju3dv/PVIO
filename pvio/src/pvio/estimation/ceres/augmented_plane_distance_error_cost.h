...
**************************************************************************/
#ifndef PVIO_AUGMENTED_PLANE_DISTANCE_ERROR_COST_H
#define PVIO_AUGMENTED_PLANE_DISTANCE_ERROR_COST_H
...
class AugmentedPlaneDistanceErrorCost : public ceres::CostFunction {
  public:
    AugmentedPlaneDistanceErrorCost(const Track *track, double sqrt_inv_cov, double regularization_weight = 1.0) :
    ...
    bool Evaluate(const double *const *parameters, double *residuals, double **jacobians) const override {
        matrix<> A;
        vector<> b;
        A.resize(keypoints.size() * 2 + 1, 3);
        b.resize(keypoints.size() * 2 + 1);
        ...
        A.row(keypoints.size() * 2) = regularization_weight * normal;
        b(keypoints.size() * 2) = regularization_weight * distance;
        ...
        if (jacobians) {
            ...
            if (double *ptr = jacobians[2 * keypoints.size() + 1]) {
                double &drdd(ptr[0]);
                drdd = -(regularization_weight * normal.dot(ATAinv * A.row(keypoints.size() * 2).transpose()) + 1);
                drdd *= sqrt_inv_cov;
            }
        }
...
#endif // PVIO_AUGMENTED_PLANE_DISTANCE_ERROR_COST_H
