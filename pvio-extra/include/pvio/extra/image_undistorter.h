...
**************************************************************************/
#ifndef PVIO_PC_IMAGE_UNDISTORTER_H
#define PVIO_PC_IMAGE_UNDISTORTER_H
...
class ImageUndistorter {
  public:
    ImageUndistorter(size_t width, size_t height, const matrix<3>& K, const std::vector<double>& distort_coeffs, const std::string& model) :
        width(width), height(height), K(K), distort_coeffs(distort_coeffs), model(model) {
        ...
        for (size_t v = 0; v < height; ++v) {
            for (size_t u = 0; u < width; ++u) {
                ...
                map_x_float.at<float>(v, u) = static_cast<float>(distorted_location.x());
                map_y_float.at<float>(v, u) = static_cast<float>(distorted_location.y());
            }
        }
    ...
    vector<2> distort_pixel(const vector<2>& undistort_location) {
        ...
        Eigen::Vector3d norm_pixel_location = K.inverse() * vector<3>(undistort_location.x(), undistort_location.y(), 1.0);
